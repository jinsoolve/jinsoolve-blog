...
// SearchBox 컴포넌트
const SearchBox = ({ onQueryChange, isMobile, autoFocus = false }: {
  ...
  const submitQuery = (value: string, immediately = false) => {
    if (debounceTimer.current) clearTimeout(debounceTimer.current);
    ...
    if (immediately) submit();
    else debounceTimer.current = setTimeout(submit, 200);
  };
...
const SearchRuntime: React.FC<SearchRuntimeProps> = ({
  initialOpen = false,
  initialFocused = false,
}) => {
  const [isFocused, setIsFocused] = useState(initialFocused);
  const [query, setQuery] = useState("");
  const [isSearchOpen, setIsSearchOpen] = useState(initialOpen);
